...
MACHINE="$(uname -m)"
BYTEORDER="$(python3 -c 'import sys; print(sys.byteorder)')"
POINTER_BITS="$(python3 -c 'import struct; print(struct.calcsize("P") * 8)')"
case "$MACHINE" in
    armv7l|armv7*) ARCH="armv7" ;;
    aarch64|arm64) ARCH="arm64" ;;
    mips*|MIPS*)
        if [ "$BYTEORDER" = "little" ] && [ "$POINTER_BITS" = "32" ]; then
            ARCH="mipsel"
        else
            echo "ERROR: Unsupported MIPS userspace: machine=$MACHINE byteorder=$BYTEORDER bits=$POINTER_BITS"
            exit 1
        fi
        ;;
    *)
...
if ! wget -q --no-check-certificate "${VERSION_URL}?cb=${CACHE_BUST}" -O "$VERSION_FILE"; then
